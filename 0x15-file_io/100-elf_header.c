 ...
 *
 * Description: If the file not be an ELF file - exit code 98.
 */
void check_elf(unsigned char *e_ident)
{
	int index;
...
/**
 * print_class - This function displays the class of
 * an ELF header.
...
/**
 * close_elf - Function close an ELF file.
 * @elf: The file descriptor of ELF file.
 *
 * Description: If the file refused to be closed - exit code 98.
 */
void close_elf(int elf)
{
	if (close(elf) == -1)
 ...
 * Return: 0 on success.
 *
 * Description: If the file is not equal to
 *  an ELF File either
